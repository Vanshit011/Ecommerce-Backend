...
export class AddPraentChildCategory1769756062133 implements MigrationInterface {
  name = 'AddPraentChildCategory1769756062133';
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."orders_status_enum_old" AS ENUM('Pending', 'Confirmed', 'Shipped', 'Delivered', 'Cancelled', 'pending', 'confirmed', 'shipped', 'delivered', 'cancelled', 'failed')`,
    );
    await queryRunner.query(
      `ALTER TABLE "orders" ALTER COLUMN "status" DROP DEFAULT`,
    );
